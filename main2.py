...
def nnan(answers):
    ...
    answers['1'] = nn2n.ABAC_model(answers['1'], batch=0, sm=sm, md=md)
...
def generate_music(sm: SettingsManager,
    ...
    answers = {'1':[], '2':[], '3':[]}
    revise_model = torch.load('./model/revise_model.pth', map_location='cuda:0').to(device)
    ...
    for i in range(len(music_bass)//8):
        ...
        answers['1'].append(list(nnan_pred[0].cpu().detach().numpy()))
