...
class MultiHead_nnan(nn.Module):
...
class PositionWiseFFN_nnan(nn.Module):
...
class EncoderLayer_nnan(nn.Module):

    def __init__(self, n_head, emb_dim, drop_rate):
        super().__init__()
        self.mh = MultiHead_nnan(n_head, emb_dim, drop_rate)
        self.ffn = PositionWiseFFN_nnan(emb_dim, drop_rate)
...
class Encoder_nnan(nn.Module):
    def __init__(self, n_head, emb_dim, drop_rate, n_layer):
        super().__init__()
        self.encoder_layers = nn.ModuleList(
            [EncoderLayer_nnan(n_head, emb_dim, drop_rate) for _ in range(n_layer)]
        )
...
class DecoderLayer_nnan(nn.Module):
    def __init__(self, n_head, model_dim, drop_rate):
        super().__init__()
        self.mh = nn.ModuleList([MultiHead_nnan(n_head, model_dim, drop_rate) for _ in range(2)])
        self.ffn = PositionWiseFFN_nnan(model_dim, drop_rate)
...
class Decoder_nnan(nn.Module):
    def __init__(self, n_head, model_dim, drop_rate, n_layer):
        ...
        self.decoder_layers = nn.ModuleList(
            [DecoderLayer_nnan(n_head, model_dim, drop_rate) for _ in range(n_layer)]
        )
...
class PositionEmbedding_nnan(nn.Module):
...
class Transformer_nnan(nn.Module):
    def __init__(self, n_vocab, max_len, n_layer=6, emb_dim=512, n_head=8, drop_rate=0.1, padding_idx=0):
        ...
        self.embed = PositionEmbedding_nnan(max_len, emb_dim, n_vocab)
        self.embedbass = PositionEmbedding_nnan(max_len, emb_dim, n_vocab)
        self.encoder = Encoder_nnan(n_head, emb_dim, drop_rate, n_layer)
        self.encoder2 = Encoder_nnan(n_head, emb_dim, drop_rate, n_layer)
        self.decoder = Decoder_nnan(n_head, emb_dim, drop_rate, n_layer)
...
if __name__ == '__main__':
    ...
    n_head = 4
    MAX_LEN = 10
    # nnan_model = Transformer_nnan(n_vocab=num_word, max_len=MAX_LEN, n_layer=n_layer, emb_dim=emb_dim, n_head=n_head,
    ...
    for epoch in range(40000):
        for batch_idx, batch in enumerate(loader):
            ...
            if batch_idx % 20 == 0 :#
                ...
                print('xbass : ', xbass[0].shape, xbass[0])
                print('xinput: ', xinput[0].shape, xinput[0])
